# Tarea: Serialización real en service-springboot
...
Reemplazar los placeholders de serialización en service-springboot con implementaciones reales para protobuf, avro, thrift y flatbuffers.
...
El modelo User tiene 4 campos: `id` (String/UUID), `name` (String), `email` (String), `timestamp` (long).

Se usan implementaciones SIN código generado:
- **Protobuf**: CodedOutputStream/CodedInputStream manual (field 1=id, 2=name, 3=email, 4=timestamp)
- **Avro**: GenericRecord + GenericDatumWriter/Reader con schema parseado del .avsc
- **Thrift**: TCompactProtocol manual (field 1=id STRING, 2=name STRING, 3=email STRING, 4=timestamp I64)
- **FlatBuffers**: FlatBufferBuilder manual (tabla con 4 campos)
...
También copiar el schema Avro al classpath:

### service-springboot/src/main/resources/avro/message.avsc
...
```bash
grep -q "serializeProtobuf" service-springboot/src/main/java/com/serialplab/springboot/serialization/SerializationService.java && grep -q "serializeAvro" service-springboot/src/main/java/com/serialplab/springboot/serialization/SerializationService.java && test -f service-springboot/src/main/resources/avro/message.avsc && echo "OK"
```
