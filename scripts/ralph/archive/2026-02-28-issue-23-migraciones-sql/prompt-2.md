# Tarea: Serialización real en service-quarkus
...
Reemplazar los placeholders de serialización en service-quarkus con implementaciones reales. Mismo patrón que service-springboot pero con anotación `@ApplicationScoped`.

## Ficheros a modificar/crear

- `service-quarkus/src/main/java/com/serialplab/quarkus/serialization/SerializationService.java`
- `service-quarkus/src/main/resources/avro/message.avsc`

## Contexto

Idéntico a service-springboot pero con `@ApplicationScoped` en lugar de `@Service`. Usa las mismas librerías (protobuf-java, avro, libthrift, flatbuffers-java).

### Contenido COMPLETO de SerializationService.java
...
public class SerializationService {
    ...
    private Map<String, Object> deserializeFlatBuffers(byte[] data) {
        ...
        if (vtableSize > 4) {
            int f = buf.getShort(vtableOffset + 4) & 0xFFFF;
            if (f != 0) result.put("id", readFbString(buf, rootTable + f));
        }
        if (vtableSize > 6) {
            int f = buf.getShort(vtableOffset + 6) & 0xFFFF;
            if (f != 0) result.put("name", readFbString(buf, rootTable + f));
        }
        if (vtableSize > 8) {
            int f = buf.getShort(vtableOffset + 8) & 0xFFFF;
            if (f != 0) result.put("email", readFbString(buf, rootTable + f));
        }
        if (vtableSize > 10) {
            int f = buf.getShort(vtableOffset + 10) & 0xFFFF;
            if (f != 0) result.put("timestamp", buf.getLong(rootTable + f));
        }
        return result;
    }
...
```bash
grep -q "serializeProtobuf" service-quarkus/src/main/java/com/serialplab/quarkus/serialization/SerializationService.java && grep -q "serializeAvro" service-quarkus/src/main/java/com/serialplab/quarkus/serialization/SerializationService.java && test -f service-quarkus/src/main/resources/avro/message.avsc && echo "OK"
```
